...
def get_ground_truth(G, gt_file_path="/home/toto/Eska/Data/Yeast_GT.txt"):
    ground_truth_partition_dict = {}
    ...
    with open(gt_file_path, 'r') as f:
        for idx, line in enumerate(f):
            ...
            nodes = [str(tok) for tok in nodes_str.strip().split()]
            for node in nodes:
                ground_truth_partition_dict[node] = idx
            ground_truth_communities.append(set(nodes))

    # Make list aligned with G nodes (strings)
    nodes_list_g = list(G.nodes())
    ground_truth_gt_partition = [
        ground_truth_partition_dict.get(node, -1) for node in nodes_list_g
    ]

    return ground_truth_partition_dict, ground_truth_gt_partition, ground_truth_communities
...
def run_fuzzy_lpa(G, max_iter=100, threshold=0.01, lambd=5.0):
...
def run_fuzzy_ldpa(G, max_iter=100, threshold=0.2, lambd=1.0):
...
def fuzzy_BLDLP(G, alpha=0.5, lambd=2.0, threshold=0.3, max_iter=100):
    """
...
def Hybrid_CMeans(G, m=1.5, epsilon=1e-4, max_iter=100, lower_thr=0.6, upper_thr=0.3, K_param=38):
    """
...
# ---------- EVALUATE COMMUNITIES (perbaikan besar) ----------
...
methods = {
    ...
    "CFinder":          lambda G: CFinder(G, k=5),
    "LFK":              LFK,
    "Hybrid C-Means":   lambda G: Hybrid_CMeans(G, K_param=38),
    "NMG":              NMG,
}
results = []
...
for method_name, method_func in methods.items():
    ...
    for run in range(n_runs):
        try:
            ...
            communities_dict = {}
            memberships_pred = None # Default untuk algoritma non-fuzzy

            # --- Standarisasi Penanganan Output ---
            if isinstance(result, tuple):
                # Hasil dari fuzzy BLDLP, LPA, LDPA, C-Means (comm_dict, membership_dict)
                communities_dict = result[0]
                if len(result) > 1 and isinstance(result[1], dict):
                    memberships_pred = result[1] # TANGKAP MEMBERSHIP

            elif isinstance(result, dict):
                # Hasil dari CFinder, LFK, NMG (hanya comm_dict)
                communities_dict = result

            else:
                 # Tangani output yang tidak terstandar (misalnya list of lists)
                if isinstance(result, list):
                     communities_dict = {i: comm for i, comm in enumerate(result) if len(comm) > 2}
                else:
                    raise TypeError(f"Output {method_name} tidak valid.")

            # Filter komunitas yang terlalu kecil
            communities_dict = {k: v for k, v in communities_dict.items() if len(v) > 2}


            # Evaluasi
            if communities_dict:
                # KIRIM memberships_pred ke fungsi evaluasi
                metrics = evaluate_communities(communities_dict, ground_truth, G, memberships_pred) # <--- KOREKSI
                ...
                CommF_list.append(metrics['Comm_F1'])
                 ...
                 # Jika tidak ada komunitas yang terdeteksi
                R_list.append(0.0)
                P_list.append(0.0)
                F_list.append(0.0)
...
out_path = "/home/toto/Eska/result_fuzzy_methods_yeast.csv"
